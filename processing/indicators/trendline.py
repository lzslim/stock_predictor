...
def macd(df, n=12, m=26, k=9):
    ...
    """
    _macd = pd.DataFrame()
    _macd['trade_date'] = df['trade_date']
...
def vrsi(df, n=6):
    ...
    """
    _vrsi = pd.DataFrame()
    _vrsi['trade_date'] = df['trade_date']
...
def boll(df, n=26, k=2):
    ...
    """
    _boll = pd.DataFrame()
    _boll['trade_date'] = df.trade_date
...
def bbiboll(df, n=10, k=3):
    ...
    _bbiboll['trade_date'] = df.trade_date
...
def bias(df, n=12):
    ...
    """
    _bias = pd.DataFrame()
    _bias['trade_date'] = df.trade_date
...
def vr_rate(df, n=26):
    ...
    """
    _vr = pd.DataFrame()
    _vr['trade_date'] = df['trade_date']
    _m = pd.DataFrame()
    _m['vol'] = df.vol
    _m['cs'] = df.close - df.close.shift(1)
    _m['avs'] = _m.apply(lambda x: x.vol if x.cs > 0 else 0, axis=1)
    _m['bvs'] = _m.apply(lambda x: x.vol if x.cs < 0 else 0, axis=1)
    _m['cvs'] = _m.apply(lambda x: x.vol if x.cs == 0 else 0, axis=1)
...
def vr(df, n=5):
    ...
    _vr = pd.DataFrame()
    _vr['trade_date'] = df.trade_date
    _vr['vr'] = df.vol / _ma(df.vol, n).shift(1)
...
def arbr(df, n=26):
    ...
    """
    _arbr = pd.DataFrame()
    _arbr['trade_date'] = df.trade_date
...
def trix(df, n=12, m=20):
    ...
    """
    _trix = pd.DataFrame()
    _trix['trade_date'] = df.trade_date
...
def dbcd(df, n=5, m=16, t=76):
    ...
    """
    _dbcd = pd.DataFrame()
    _dbcd['trade_date'] = df.trade_date
    man = _ma(df.close, n)
...
def vroc(df, n=12):
    ...
    """
    _vroc = pd.DataFrame()
    _vroc['trade_date'] = df['trade_date']
    _vroc['vroc'] = (df.vol - df.vol.shift(n)) / df.vol.shift(n) * 100
    return _vroc
...
def pvt(df):
    ...
    """
    _pvt = pd.DataFrame()
    _pvt['trade_date'] = df.trade_date

    x = (df.close - df.close.shift(1)) / df.close.shift(1) * df.vol
...
def wvad(df, n=24, m=6):
    ...
    """
    _wvad = pd.DataFrame()
    _wvad['trade_date'] = df.trade_date
    # _wvad['wvad'] = (np.true_divide((df.close - df.open), (df.high - df.low)) * df.vol).rolling(n).sum()
    _wvad['wvad'] = (np.true_divide((df.close - df.open), (df.high - df.low)) * df.vol).rolling(n).sum()
...
def mike(df, n=12):
    ...
    """
    _mike = pd.DataFrame()
    _mike['trade_date'] = df.trade_date
...
def vma(df, n=5):
    """
    量简单移动平均	VMA(5)	VMA=MA(vol,N)
    vol表示成交量；N表示天数
    """
    _vma = pd.DataFrame()
    _vma['trade_date'] = df.trade_date
    _vma['vma'] = _ma(df.vol, n)
    return _vma
...
def vmacd(df, qn=12, sn=26, m=9):
    ...
    """
    _vmacd = pd.DataFrame()
    _vmacd['trade_date'] = df.trade_date
    _vmacd['diff'] = _ema(df.vol, qn) - _ema(df.vol, sn)
...
def tapi(df, n=6):
    ...
    """
    _tapi = pd.DataFrame()
    # _tapi['trade_date'] = df.trade_date
...
def vstd(df, n=10):
    """
    成交量标准差	vstd(10)
    VSTD=STD（vol,N）=[∑（vol-MA(vol，N)）^2/N]^0.5
    """
    _vstd = pd.DataFrame()
    _vstd['trade_date'] = df.trade_date
    _vstd['vstd'] = df.vol.rolling(n).std(ddof=1)
    return _vstd
...
def adtm(df, n=23, m=8):
    ...
    """
    _adtm = pd.DataFrame()
    _adtm['trade_date'] = df.trade_date
    _m = pd.DataFrame()
...
def micd(df, n=3, m=10, k=20):
    ...
    """
    _micd = pd.DataFrame()
    _micd['trade_date'] = df.trade_date
...
def rccd(df, n=59, m=21, k=28):
    ...
    """
    _rccd = pd.DataFrame()
    _rccd['trade_date'] = df.trade_date
...
def srmi(df, n=9):
    ...
    """
    _srmi = pd.DataFrame()
    _srmi['trade_date'] = df.trade_date
    _m = pd.DataFrame()
    _m['close'] = df.close
...
def dptb(df, n=7):
    ...
    _dptb = pd.DataFrame(index=df.trade_date)
...
def jdqs(df, n=20):
    ...
    sd.set_index('trade_date', inplace=True)   # 可能出现停盘等情况，所以将trade_date设为index
    ind.set_index('trade_date', inplace=True)
    _jdrs = pd.DataFrame(index=df.trade_date)
    q = ind.close - ind.open
...
def zdzb(df, n=125, m=5, k=20):
    ...
    """
    _zdzb = pd.DataFrame()
    _zdzb['trade_date'] = df.trade_date
...
def mass(df, n=9, m=25):
    ...
    """
    _mass = pd.DataFrame()
    _mass['trade_date'] = df.trade_date
...
def cvlt(df, n=10):
    ...
    """
    _cvlt = pd.DataFrame()
    _cvlt['trade_date'] = df.trade_date
...
def down_n(df):
    ...
    """
    _down = pd.DataFrame()
    _down['trade_date'] = df.trade_date
    p = df.close - df.close.shift()
    p[p > 0] = 0
    p[p < 0] = 1
    m = []
...
def join_frame(d1, d2, column='trade_date'):
    # 将两个DataFrame 按照trade_datetime合并
...
if __name__ == "__main__":
    ...
    # print(trix(data))
    # print(bbi(data))
    # print(ts.top_list(trade_date="2019-01-17"))
    # print(mtm(data))
    # print(obv(data))
    # print(cci(data))
    # print(priceosc(data))
